...
# Order matters, and it is the opposite of what it looks like. The vendored
# tree contains a *copy* of ``spektr/``, so putting it first shadows the real
# package for the whole test session — every other test file would then be
# exercising the APK's copy rather than the source, and would fail or, worse,
# pass against the wrong code. The repo root goes first so ``spektr`` is the
# real one; the vendored path is only there for ``spektr_android``, which
# exists nowhere else.
