...
def _ctx(state, f, bands, onset=0, strength=0.0, w=120, h=40, pal=PAL):
    return Ctx(w=w, h=h, bands=bands, peaks=bands, bands_l=bands, bands_r=bands,
               ...
               state=state, onsets=onset, onset_strength=strength)
...
def test_the_spokes_are_never_blanked():
    state, frames = _run(120, _groove, keep=True)
    walls = _state(state, "tunnel_geo")["walls"]
    for codes in frames[-40:]:
        assert np.array_equal(_dots(codes) & walls, walls), "a spoke dot was dropped"
...
def test_the_centre_converges_like_the_tunnels(size):
    ...
    for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
        ...
        assert all(geo["walls"][y, x] or geo["walls"][y, min(dc - 1, x + 1)] or geo["walls"][min(dr - 1, y + 1), x]
                   for y, x in ray), f"{size}: an axis spoke stops short of the centre"
