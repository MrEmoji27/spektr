...
def _cab_polar(qh: int, qw: int) -> np.ndarray:
    """Distance from a quadrant's centre, normalised so 1.0 is the cone rim.

    Cached per size: this is the only geometry Cabinet needs and it is the
    same array for all four cones, so one quadrant-sized grid does the work
    four full-grid passes would otherwise cost.
    """
    cy, cx = (qh - 1) * 0.5, (qw - 1) * 0.5
    dy = (np.arange(qh, dtype=np.float32) - cy)[:, None]
    dx = (np.arange(qw, dtype=np.float32) - cx)[None, :]
    r = max(1.0, min(cy, cx) * 0.86)
    return np.sqrt(dy * dy + dx * dx) / np.float32(r)


@mode("Cabinet", group="scenes", blurb="a 4x12 guitar cab; the cones clip when the tone does")
def cabinet(ctx: Ctx):
    """A speaker cabinet head-on, driven by how *distorted* the sound is.

    Every other mode in spektr reads level and rough band position. Neither can
    tell a clean guitar from an overdriven one at the same volume, which is a
    problem if you want a mode that belongs to a genre rather than to a volume
    knob. This one reads ``ctx.flatness`` — the geometric over the arithmetic
    mean of the band magnitudes. Clipping generates dense harmonics, so a wall
    of distorted guitar drives the spectrum noise-like and flat while a clean
    tone stays peaky, and that difference is what the picture is about.

    Four cones in a 2x2, each carrying a quarter of the spectrum. Concentric
    ripples travel out from each dust cap, their phase accumulated through
    ``ctx.dt`` and their speed set by that cone's band, so a loud cone visibly
    works harder rather than just glowing brighter.

    Drive does the thing the mode is named for: the ripple is a sine, and at
    high flatness it is multiplied up and clipped, so the smooth gradient
    squares off into hard-edged bands. That is not a metaphor for clipping —
    it is the same operation, applied to the waveform being drawn.

    Kicks shake the whole cab. The shake is an impulse with a ``ctx.dt`` decay
    and a direction that is fixed for the life of that shake, never per-frame
    jitter, which would strobe. It is applied by offsetting where each cone's
    block is *written* rather than by re-indexing the grid, so a full-frame
    shake costs slice arithmetic instead of a gather over every dot.
    """
    dr, dc = ctx.dot_rows, ctx.dot_cols
    if dr < 24 or dc < 32:
        return empty(ctx.w, ctx.h)
    ...
    qh, qw = dr // 2, dc // 2
    pol = ctx.scratch("cab_polar", lambda: _cab_polar(qh, qw))
    ...
    lv = ctx.display_bands(4).astype(np.float64)
    bass = ctx.range(0.0, 0.2)
    st["fast"] += (bass - st["fast"]) * min(1.0, ctx.dt / 0.03)
    st["slow"] += (bass - st["slow"]) * min(1.0, ctx.dt / 0.40)

    # Drive, rescaled off the flatness floor. A real spectrum never reaches 0
    # flatness — the quiet bands sit on a noise floor rather than at zero — so
    # the usable range in practice runs from about 0.35 for tonal material to
    # near 1.0 for a wall of gain, and this maps that onto 0..1.
    drive = float(np.clip((ctx.flatness - 0.35) / 0.50, 0.0, 1.0))

    st["shake"] *= math.exp(-max(ctx.dt, 0.0) / 0.10)
    if (st["fast"] - st["slow"]) > 0.10 and st["shake"] < 0.35:
        ang = float(st["rng"].uniform(0.0, 2 * math.pi))
        st["dir"] = (math.sin(ang), math.cos(ang))
        st["shake"] = 1.0
    kick = st["shake"] * (0.02 + 0.05 * drive)
    sy = int(round(st["dir"][0] * kick * qh))
    sx = int(round(st["dir"][1] * kick * qw))

    out = np.zeros((dr, dc), dtype=np.float32)
    tau = np.float32(2.0 * math.pi)
    for j in range(4):
        level = float(lv[j])
        st["phase"][j] += (5.0 + 16.0 * level) * ctx.dt
        w = np.sin(pol * (tau * np.float32(2.4)) - np.float32(st["phase"][j]))
        # The clip. Multiplying up and clamping is exactly what an overdriven
        # amp does to a waveform, and it turns the smooth ripple into hard
        # bands on screen for the same reason it turns a sine into a square.
        if drive > 0.02:
            np.clip(w * np.float32(1.0 + drive * 7.0), -1.0, 1.0, out=w)

        cone = pol <= 1.0
        val = cone * np.float32(0.20 + 0.80 * level) * (np.float32(0.45) + np.float32(0.55) * w)
        np.maximum(val, (np.abs(pol - 1.0) < 0.05) * np.float32(0.55), out=val)  # surround
        np.maximum(val, (pol < 0.20) * np.float32(0.70 + 0.30 * level), out=val)  # dust cap

        y0 = (j // 2) * qh + sy
        x0 = (j % 2) * qw + sx
        ty0, tx0 = max(0, y0), max(0, x0)
        ty1, tx1 = min(dr, y0 + qh), min(dc, x0 + qw)
        if ty1 <= ty0 or tx1 <= tx0:
            continue
        out[ty0:ty1, tx0:tx1] = np.maximum(
            out[ty0:ty1, tx0:tx1], val[ty0 - y0 : ty1 - y0, tx0 - x0 : tx1 - x0]
        )

    # Grille cloth between the cones, so the cab is a box and not four floating
    # discs. A regular weave was the obvious thing and it was wrong: any small
    # fixed period aliases against the 2x4 braille cell and renders as graph
    # paper — one repeating glyph across the whole screen, loud enough to bury
    # the cones. Fixed speckle reads as cloth, and thresholding it against a
    # level that rises with drive means the grille comes up as the amp is
    # pushed instead of being a constant backdrop.
    weave = ctx.scratch(
        "cab_weave",
        lambda: np.random.default_rng(613).random((dr, dc)).astype(np.float32),
    )
    # Density is the threshold itself, so it stays sparse: a braille cell reads
    # as filled if any one of its eight dots is lit, so even a 20%-dense
    # speckle covers 83% of cells and swamps the cones. Around a tenth is
    # enough to say "there is cloth here" and leave the picture to the drivers.
    cloth = np.float32(0.05 + 0.10 * ctx.energy + 0.13 * drive)
    np.maximum(out, (weave < cloth) * cloth, out=out)

    # cabinet edge, shaken with everything else
    ey0, ex0 = max(0, sy), max(0, sx)
    ey1, ex1 = min(dr, dr + sy) - 1, min(dc, dc + sx) - 1
    rim = np.float32(0.45 + 0.55 * drive)
    out[ey0, ex0 : ex1 + 1] = rim
    out[ey1, ex0 : ex1 + 1] = rim
    out[ey0 : ey1 + 1, ex0] = rim
    out[ey0 : ey1 + 1, ex1] = rim

    dots = out > 0.10
    codes = pack_braille(dots)
    cidx = ctx.ramp(cell_max(np.where(dots, out, 0.0)))
    return codes, cidx
